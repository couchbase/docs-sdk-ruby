...
The Data Service exposes several common errors that can be encountered -- both during development, and to be handled by the production app. 
...
xref:{version-server}@server:learn:security/authorization-overview.adoc[Refer to our Authorization pages] for a fuller understanding.
