...
Note that the type returned by `#expiry_time` is `Time`, and always represents absolute time when the document will expire. Method `#expiry`
that returned integer number of seconds since epoch is deprecated and will be removed in release `3.1`.
