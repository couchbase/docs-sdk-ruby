...
IMPORTANT: The SDK uses an https://github.com/ruby/openssl[OpenSSL wrapper] for TLS functionality, which loads your system's certificate store by default.
If your system does not have a default root CA store, it will use the https://ruby.github.io/openssl/OpenSSL/X509/Store.html[Mozilla CA] certificate store.
