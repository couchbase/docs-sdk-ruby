...
By default it uses built-in means to report events.  The default log level is `warning`.
...
The Ruby SDK is configured to send logs to standard output. The logging level can be changed using environment variable
`COUCHBASE_BACKEND_LOG_LEVEL`.
