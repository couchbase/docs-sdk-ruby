...
Implement `RawJsonTranscoder`, `RawBinaryTranscoder` and `RawStringTranscoder` & check flags when decoding document
content (https://github.com/couchbase/couchbase-ruby-client/pull/93[#93]).

* https://issues.couchbase.com/browse/RCBC-427[RCBC-427]:
Add missing query index management options & use `InvalidArgument` error
(https://github.com/couchbase/couchbase-ruby-client/pull/92[#92]).
Add missing options `scope_name`, `collection_name` and `index_name` for `#create_primary` method.

* https://issues.couchbase.com/browse/RCBC-436[RCBC-436]:
Always use PLAIN SASL mechanism with TLS connections
...
* Add constructor for `SearchRowLocation`
(https://github.com/couchbase/couchbase-ruby-client/pull/95[#95]).

* Change `attr` to `attr_reader`
...
* Introduce connection string option `dump_configuration` for debugging
(https://github.com/couchbaselabs/couchbase-cxx-client/pull/398[#398]).
It logs cluster configuration on trace level.
