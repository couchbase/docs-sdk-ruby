...
[source,ruby]
----
res = collection.lookup_in("user:kingarthur", [
...
options.project(["contact.name", "contact.email"])
res = collection.get("user:kingarthur", options)
res.content
#=> {"contact"=>{"email"=>"kingarthur@couchbase.com", "name"=>"King Arthur"}}
----
